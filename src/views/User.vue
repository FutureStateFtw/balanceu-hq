    ...
    <v-container fluid class="pa-4 pa-md-6">
                    ...
                    <div class="card-inner">
                        <div class="d-flex align-center justify-space-between mb-1">
                            <div class="d-flex align-center">
                                ...
                                <div class="text-body-2 text-white-subtitle">{{ bal.label }}</div>
                            </div>
                            <v-btn v-if="bal.canDeposit" variant="text" size="small" class="text-caption text-white-subtitle" @click.stop="performBalanceAction(bal)">
                        ...
                        <div v-if="bal.availableThisWeek" class="text-caption text-white-muted d-flex justify-end">Available This Week&nbsp;<strong>{{ bal.availableThisWeek }}</strong></div>
                        ...
                        <v-expand-transition>
                            <div v-show="balanceDropdowns[bal.key]" class="mt-3">
                                ...
                                <div class="text-caption text-white-subtitle mb-2">Recent {{ bal.label }} Activity</div>
                                <div v-if="getBalanceTransactions(bal.key).length === 0" class="text-caption text-white-muted py-2 text-center">
                                ...
                                <div v-for="t in getBalanceTransactions(bal.key)" :key="t.id" class="balance-transaction-row">
                                    <div class="d-flex justify-space-between align-center py-2">
                                        <div class="flex-grow-1">
                                            <div class="text-caption text-white-muted">{{ t.merchant }}</div>
                                            <div class="text-caption text-white-muted" style="font-size: 0.7rem; opacity: 0.7;">
                                                {{ formatTransactionDate(t.date) }}
                                            </div>
                                        </div>
                                        <div class="text-caption" :class="t.amount > 0 ? 'text-green-lighten-2' : 'text-white'">
            ...
            <v-col cols="12">
                <div class="std-glass-panel">
                    <div class="px-5 py-6 d-flex align-center justify-space-between" @click="transactionsOpen = !transactionsOpen" style="cursor: pointer;">
                        <div class="text-subtitle-2 font-weight-medium text-white-subtitle">All Transactions</div>
                        <v-icon :icon="transactionsOpen ? 'mdi-chevron-up' : 'mdi-chevron-down'" size="18" class="text-white-muted"></v-icon>
                    </div>
                    <v-expand-transition>
                            ...
                            >
                                <template v-slot:default="{ item }">
                                    <div class="transaction-row">
                                        <div class="merchant">
                                            <div class="text-body-2 font-weight-medium text-white">{{ item.merchant }}</div>
        ...
        <v-row class="mb-12">
            <v-col v-for="qa in quick" :key="qa.key" cols="12" sm="4" class="d-flex">
                <div class="std-glass-action-card quick-action-card flex-grow-1 pa-6 text-center" @click="triggerQuickAction(qa)" @mouseenter="hoverAction($event)" @mouseleave="leaveAction($event)">
                    ...
                    <div class="text-body-2 font-weight-medium text-white">{{ qa.label }}</div>
                    <div class="std-glass-border-glow"></div>
                </div>
            </v-col>
        </v-row>
        ...
        <div class="bottom-nav-wrapper" :class="{ open: bottomNavOpen }">
            <transition name="std-fade-slide">
                <div v-if="bottomNavOpen" class="std-glass-panel bottom-nav-sheet">
                    <div v-for="act in bottomNav" :key="act.key" class="bottom-nav-item" @click="handleBottomAction(act)">
                        ...
                        <span class="text-body-2 text-white">{{ act.label }}</span>
                    </div>
                </div>
            </transition>
            <v-btn class="std-glass-button text-white text-subtitle-2 text-uppercase" color="transparent" variant="flat" size="large" @click="bottomNavOpen = !bottomNavOpen">
                {{ bottomNavOpen ? 'Close' : 'Navigate' }}
            </v-btn>
        </div>
        ...
        <v-dialog v-model="dialog" max-width="420">
            <div class="std-glass-panel pa-6">
                ...
                <div class="text-body-2 mb-6 text-white-subtitle">{{ dialogMessage }}</div>
                <div class="d-flex justify-end">
                    <v-btn variant="text" class="std-glass-button mr-2" @click="dialog=false">Cancel</v-btn>
                    <v-btn class="std-glass-button" color="primary" @click="confirmDialog">Ok</v-btn>
                </div>
            </div>
        </v-dialog>
